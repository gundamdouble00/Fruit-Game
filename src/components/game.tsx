...
const texts = {
  vi: {
    ...
    explainNoAnswer:
      "Sau 20 thẻ bài mà không có một loại trái cây nào có tổng là 5, lượt chơi đó kết thúc.",
  ...
  en: {
    ...
    explainCorrect:
      "You selected the correct fruit within the allotted time (Under 5 seconds).",
    explainWrong1: "You selected the wrong type of fruit.",
    explainWrong2: "The fruit you selected are not exactly 5.",

    explainOutOf5s:
      "After 5 seconds, if you have not identified a fruit type that sums to 5, you lose.",

    explainLarger: "The fruit you selected exceeds 5 in total.",
    explainMiss: "You missed another fruit that added up to 5.",
    explainNoAnswer:
      "After 20 cards, if no fruit combination adds up to 5, the round ends.",
...
export default function Game({ language }: GameProps) {
        ...
        if (gameState.gameStatus === "playing") {
          setGameState((prev) => {
            ...
            currentCards.forEach((card) => {
              fruitCounts.set(
                card.fruit,
                (fruitCounts.get(card.fruit) || 0) + card.count,
              );
            ...
            const generateUniqueCard = (
              ...
              for (let i = 0; i < fullCards.length; i++) {
                ...
                for (let j = 0; j < cards.length; j++) {
                  if (
                    fullCards[i].fruit === cards[j].fruit &&
                    fullCards[i].count === cards[j].count
                  ) {
                    isExist = true;
                    break;
                  }
  ...
  const clickFruit = useCallback(
    async (selectedFruit: FruitType) => {
      setGameState((prev) => {
        let gameSt = prev.gameStatus;
        ...
        if (selectedFruit === Answer) {
          ...
          if (fruitCounts.get(selectedFruit) === 5) {
            if (Temp <= 5) {
              ...
              msg =
                language === "vi"
                  ? texts.vi.correctAnswer
                  : texts.en.correctAnswer;
        ...
        if (selectedFruit !== Answer) {
          if (Answer !== undefined) {
            ...
            if (Fruits === 5) {
              ...
              msg =
                language === "vi" ? texts.vi.missAnswer : texts.en.missAnswer;
              bgColor = "bg-yellow-100";
  ...
  const renderCards = () => {
    return gameState.cards.map((card, index) => (
      <Card key={index} className="relative h-28 w-20">
  ...
  const renderFruitButtons = () => {
    ...
    return buttons.map(({ fruit, icon }) => (
      <Button
        ...
        // className="p-4"
        className="flex h-14 w-14 flex-col items-center justify-center rounded-lg bg-white shadow-md"
        variant="outline"
      >
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <Dialog open={showNotification} onOpenChange={setShowNotification}>
        <DialogContent>
          ...
          <div className="flex-2 flex-1">
            <h2 className="mb-4 text-2xl font-bold">Kết thúc 2 lượt</h2>
            <p className="mb-4">
      ...
      <Dialog
        open={showRoundNotification}
        onOpenChange={setShowRoundNotification}
      >
        <DialogContent>
          <DialogTitle></DialogTitle>
          <div className="p-6">
            ...
            <div className="flex justify-center">
              ...
              >
                ...
                {gameState.currentRound > gameState.totalRounds
                  ? language === "vi"
                    ? texts.vi.viewResults
                    : texts.en.viewResults
                  : language === "vi"
                    ? texts.vi.continue
                    : texts.en.continue}
              </Button>
      ...
      {gameState.gameStatus === "playing" && !showRoundNotification && (
        <div className="top-0 flex w-full flex-col items-center gap-8 pb-24">
          <div className="mb-16 h-auto w-full flex-1 overflow-y-auto rounded-md border border-white bg-white p-4 sm:h-[400px] sm:w-[350px]">
            <div className="grid max-w-3xl grid-cols-4 items-center justify-center gap-4">
              {renderCards()}
            </div>
          </div>

          <div className="fixed bottom-4 left-2 right-2 flex flex-col items-center gap-4 rounded-md border-white bg-orange-50 p-2 shadow-lg">
            <div className="flex gap-4">{renderFruitButtons()}</div>
            <div className="bottom-8 mt-2 items-center justify-center text-xl font-bold">
              {language === "vi" ? texts.vi.round : texts.en.round}{" "}
              {gameState.currentRound}/{gameState.totalRounds}
            </div>
          </div>
        </div>
...
const RenderResults = ({
      ...
      <div className="mt-4 border">
        <div className="top-12">
          <p className="ml-4 text-left">
            ...
            {language === "vi"
              ? texts.vi.explainCorrect
              : texts.en.explainCorrect}
          </p>
        </div>
        <div className="ml-4">
          ...
          <p className="ml-8 text-left">
            +{" "}
            {language === "vi"
              ? texts.vi.explainWrong1
              : texts.vi.explainWrong1}
          </p>
          <p className="ml-8 text-left">
            +{" "}
            {language === "vi"
              ? texts.vi.explainWrong2
              : texts.vi.explainWrong2}
          </p>
        </div>

        <div className="ml-4 text-left font-semibold text-red-300">
          {language === "vi"
            ? texts.vi.explainOutOf5s
            : texts.en.explainOutOf5s}
        </div>

        <div className="ml-4 text-left font-semibold text-yellow-400">
        ...
        <div className="top-12">
          <p className="ml-4 text-left">
            ...
            {language === "vi"
              ? texts.vi.explainNoAnswer
              : texts.en.explainNoAnswer}
          </p>
        </div>
      </div>
    );
      ...
      <div className="mt-4 border">
        ...
        <div className="ml-4">
          <p className="text-left">
            ...
            <span className="mr-2 font-semibold text-red-300">:</span>
            {language === "vi"
              ? texts.vi.explainWrong1
              : texts.en.explainWrong1}
          </p>
        </div>
        ...
        <div className="ml-4">
          <p className="text-left">
            ...
            <span className="mr-2 font-semibold text-yellow-400">:</span>
            {language === "vi"
              ? texts.vi.explainWrong1
              : texts.en.explainLarger}
          </p>
        </div>
  ...
  return (
    <div className="mx-auto mt-8 w-full max-w-2xl">
        ...
        <thead>
          <tr className="bg-fuchsia-50">
            ...
            <th className="border p-2">
              {/* Thời gian phản ứng */}
              {language === "vi"
                ? texts.vi.reactionTime
                : texts.en.reactionTime}
            </th>
            <th className="border p-2">
              {/* Đạt yêu cầu */}
              {language === "vi"
                ? texts.vi.requirements
                : texts.en.requirements}
            </th>
          </tr>
        </thead>
        <tbody>
          {gameState.results.map((result, index) => (
            <tr key={index}>
              ...
              <td className="border bg-white p-2 text-center">
                {result.reactionTime
                  ? `${result.reactionTime.toFixed(3)}s`
                  : "-"}
              </td>
              <td className={`border p-2 text-center ${result.bgColor}`}>
                {result.msg}
              </td>
            </tr>
          ))}
      ...
      <footer className="mb-8 mt-8 text-center">
